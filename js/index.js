let SPEED = 0.25
class Game{
    constructor(gameElement, bird, obstacle, scoreElements,highScoreElements,gameSummaryElement, restartElement,instructions){
    ...
    setHighscore = (highScoreElements) =>  {
        ...
        highScoreElements.forEach(element=>element.innerHTML=localStorage.getItem("Highscore"))
    }
    ...
    detectCollision = ()=>{
        ...
        if(birdPosition.rightValue + obstaclePosition.topBlockLeftValue<=100 && birdPosition.rightValue + obstaclePosition.topBlockLeftValue>=90){
            //collision if obstacle at top is touched or obstacle at bottom is touched
            if(birdPosition.topValue <= obstaclePosition.topBlockHeightValue || birdPosition.topValue + 5 + obstaclePosition.bottomBlockHeightValue>100){
                this.stopGame()
            }
        }
    }
    ...
    checkHighScore = ()=>{
        if(Number(localStorage.getItem("Highscore"))<this.score)localStorage.setItem("Highscore",this.score)
    }
    stopGame = ()=>{
...
class Bird{
    ...
    setJump = (event)=>{
        ...
        if(event.key=="a" && topValue<90 && topValue>1){
...
class Obstacle{
    ...
    getObstaclePosition = function(){
        let topBlockLeftValue = Number(this.topBlockElement.style.left.replace(/%/g,""))
        let topBlockHeightValue = Number(this.topBlockElement.style.height.replace(/%/g,""))
        let bottomBlockLeftValue = Number(this.bottomBlockElement.style.left.replace(/%/g,""))
        let bottomBlockHeightValue = Number(this.bottomBlockElement.style.height.replace(/%/g,""))
        return {topBlockLeftValue,bottomBlockLeftValue,topBlockHeightValue,bottomBlockHeightValue}
    }
    ...
    obstacleHeightRandomizer = ()=>{
        ...
        if(topObstacleHeight+bottomObstacleHeight!=20){
            bottomObstacleHeight = bottomObstacleHeight + (70 - (topObstacleHeight + bottomObstacleHeight))
        }
    ...
    moveBlock = function(){
        ...
        if(obstaclePosition.topBlockLeftValue<=0){
            ...
            this.topBlockElement.style.height= obstacleHeights.topObstacleHeight + "%"
            this.bottomBlockElement.style.height = obstacleHeights.bottomObstacleHeight + "%"
            this.setPassed()
        }
        else{
            this.passed=false
            this.topBlockElement.style.left = obstaclePosition.topBlockLeftValue - this.SPEED + "%"
            this.bottomBlockElement.style.left = obstaclePosition.bottomBlockLeftValue - this.SPEED + "%"
        }  
    }
...
let obstacle = new Obstacle(document.getElementById("top-block"),document.getElementById("bottom-block"),90)
let game = new Game(document.getElementById("game-board"), bird, obstacle ,document.querySelectorAll(".scorevalue"),document.querySelectorAll(".highscorevalue"), gameSummaryElement,restartElement,instructions)
game.startGame()